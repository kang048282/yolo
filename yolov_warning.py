        ...
        if results[0].masks is not None:
            ...
            for i, mask in enumerate(masks):
                ...
                segmentation_results.append(
                    SegmentationResult(mask=mask, class_id=cls_id, class_name=class_name, confidence=conf)
                )
        ...
        for seg_result in seg_results:
            if seg_result.confidence > 0.3:  # 降低阈值以显示更多区域
                ...
                overlay[mask_bool] = cv2.addWeighted(
                    overlay[mask_bool],
                    0.6,  # 降低原图权重
                    np.full_like(overlay[mask_bool], color),
                    0.4,
                    0,  # 增加叠加权重
                )
                ...
                contours, _ = cv2.findContours(
                    (mask_resized > 0.3).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
                )
...
class BehaviorAnalyzer:
    ...
    def analyze_behavior(
        self, detections: List[DetectionResult], seg_results: List[SegmentationResult], track_history: Dict
    ) -> List[BehaviorAlert]:
        """分析行为并生成预警."""
        alerts = []
    ...
    def _analyze_zone_violation(
        self, detection: DetectionResult, seg_results: List[SegmentationResult], center: Tuple
    ...
    def _analyze_collision_risk(
        self, detection: DetectionResult, all_detections: List[DetectionResult], center: Tuple
    ) -> List[BehaviorAlert]:
        """分析碰撞风险."""
        alerts = []

        for other_detection in all_detections:
            if other_detection.track_id == detection.track_id or other_detection.track_id is None:
                continue
            ...
            if distance < self.proximity_threshold and (
                (detection.class_name == "person" and other_detection.class_name in ["car", "truck", "bus"])
                or (detection.class_name in ["car", "truck", "bus"] and other_detection.class_name == "person")
            ):
                alerts.append(
                    BehaviorAlert(
...
class AlertSystem:
    ...
    def get_current_display_alerts(self) -> List[BehaviorAlert]:
        """获取当前应该显示的预警."""
        return [
            alert for alert in self.persistent_alerts if hasattr(alert, "frame_counter") and alert.frame_counter > 0
        ]
...
class IntelligentTrafficAnalyzer:
    ...
    def setup_components(self):
        ...
        self.seg_manager = SegmentationManager(
            self.config["segmentation_model_path"], self.config["segmentation_data_config"]
        )

        # 初始化目标跟踪器
        self.object_tracker = ObjectTracker(self.config["detection_model_path"], self.config["deepsort_config"])
    ...
    def setup_video(self):
        ...
        self.out = cv2.VideoWriter(self.config["output_video"], fourcc, self.target_fps, (self.width, self.height))
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        # 3. 行为分析
        alerts = self.behavior_analyzer.analyze_behavior(detections, seg_results, self.object_tracker.track_history)

        # 4. 预警处理
        valid_alerts = self.alert_system.process_alerts(alerts)

        # 5. 可视化
        result_frame = self.visualize_results(frame, seg_results, detections, valid_alerts)
    ...
    def visualize_results(
        ...
        legend_y = 80
        cv2.putText(
            result_frame, "Legend:", (self.width - 200, legend_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2
        )

        # 道路图例
        cv2.rectangle(
            result_frame, (self.width - 190, legend_y + 10), (self.width - 170, legend_y + 25), (0, 200, 0), -1
        )
        cv2.putText(
            result_frame, "Road", (self.width - 160, legend_y + 22), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1
        )

        # 人行道图例
        cv2.rectangle(
            result_frame, (self.width - 190, legend_y + 35), (self.width - 170, legend_y + 50), (255, 100, 0), -1
        )
        cv2.putText(
            result_frame,
            "Sidewalk",
            (self.width - 160, legend_y + 47),
        ...
        cv2.putText(
            result_frame,
            f"Sidewalk regions: {sidewalk_count}",
            (10, self.height - 55),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (255, 100, 0),
        ...
        for alert in alerts:
            ...
            cv2.putText(result_frame, alert_text, (10, alert_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, alert_color, 2)
            alert_y += 25
        ...
        if stats:
            ...
            cv2.putText(
                result_frame, stats_text, (10, self.height - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2
            )

        # 显示帧率
        cv2.putText(result_frame, f"FPS: {int(self.fps)}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
