...
while cap.isOpened():
    ...
    for cls in [0, 1]:  # 0: car, 1: person
        if cls in gt_annotations and len(gt_annotations[cls]) > 0 and cls in all_detections and len(all_detections[cls]) > 0:
    ...
    cv2.putText(original_frame, f"FPS: {current_fps:.1f}", (10, 80), 
                cv2.FONT_HERSHEY_SIMPLEX, 3.0, (0, 255, 0), 5)  # 增大字体
    cv2.putText(original_frame, f"Conf Threshold: {conf_threshold:.2f}", (10, 160), 
                cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 0), 4)  # 显示置信度阈值
    cv2.putText(original_frame, f"Shape Diff Threshold: {shape_diff_threshold:.2f}", (10, 220), 
                cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 0), 4)  # 显示形状差异阈值
    ...
    cv2.putText(original_frame, f"Area Diff Threshold: {area_diff_threshold:.2f}", (10, 280), 
                cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 0), 4)  # 显示面积差异阈值
