...
def train(epoch):
    ...
    for idx, (inputs, labels) in enumerate(trainloader):
        ...
        if (idx + 1) % interval == 0:
            ...
            print(
                f"[progress:{100.0 * (idx + 1) / len(trainloader):.1f}%]time:{end - start:.2f}s Loss:{training_loss / interval:.5f} Correct:{correct}/{total} Acc:{100.0 * correct / total:.3f}%"
            )
            training_loss = 0.0
            start = time.time()
...
def test(epoch):
    ...
    with torch.no_grad():
        ...
        print(
            f"[progress:{100.0 * (idx + 1) / len(testloader):.1f}%]time:{end - start:.2f}s Loss:{test_loss / len(testloader):.5f} Correct:{correct}/{total} Acc:{100.0 * correct / total:.3f}%"
        )
